...
describe('route-guard', () => {
  ...
  it('only call next and none callback if path is notCallback', async () => {
    ...
    expect(authenticated).not.toBeCalled();
    expect(unauthenticated).not.toBeCalled();
    expect(next).toBeCalled();
  });

  it('calls authenticated callback if is callback', async () => {
    const authenticated = jest.fn();
    const options = createOptions({
      authenticated,
      redirectUri: '/callback',
