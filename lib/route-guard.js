...
module.exports = function routeGuard(pluginOptions, base) {
  ...
  return (to, from, next) => {
    ...
    // TODO: instead of opts.authState, loads it from local storage
    if (!opts.authState && isCallback(to, opts.redirectUri, base)) {
      opts.authenticated(to.fullPath, next);
    } else if (opts.authState) {
      opts.authenticated(opts.authState, next);
    } else {
